...
/* ═══════  Dashboard Component  ═══════ */
export default function Dashboard({ userName, appMode, onModeChange }) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">
            {greeting}, <span className={isPlacement ? 'text-purple-400' : 'text-accent-400'}>{displayName}</span>
          </h1>
          <p className="text-surface-400 text-sm">
            {isPlacement
              ? 'Here\'s an overview of your placement preparation.'
              : 'Here\'s an overview of your JEE preparation progress.'}
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            <div className={`bg-surface-800 rounded-xl border overflow-hidden ${isPlacement ? 'border-purple-500/10' : 'border-surface-700'}`}>
              <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                <Trophy className={`w-10 h-10 mb-3 ${isPlacement ? 'text-purple-500/40' : 'text-surface-600'}`} />
